...
* ==> Flow prep
************************************************************/
// Flow starts in  prerender function at the bottom
...
* ==> Prerender
************************************************************/
function preRender(placeholder) {
	if (typeof bntAd !== 'undefined' && bntAd) {
		bntAd.subscribe(function (e) {
			...
			adController.renderer.render(adController.element).map(function () {
				...
				placeholder.htmlElement.hidden = false;
...
preRender(flowConfig.image.studioElement);