...
function PercentController(config) {
  const { triggerName, element, events, loopPercent, getScrollPercent, getTimePercent, getMousePercent } = config
  ...
  const triggers = {
    ...
    onTimeAndSwipe: [getTimePercent, getMousePercent]
  }
  ...
  element.htmlElement.addEventListener(events.pointerDown, () => switchTo(currentTriggers[1]))
  element.htmlElement.addEventListener(events.pointerUp, () => switchTo(currentTriggers[0]))
  element.htmlElement.addEventListener(events.pointerCancel, () => switchTo(currentTriggers[0]))
}
